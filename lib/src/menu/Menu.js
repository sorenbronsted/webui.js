...
class Menu {
	...
	select(uri) {
		...
		if (this.uri === uri) {
			this._selected = true;
			this._active = true;
			// If this node has children then active the first node also
			if (this._children.isNotEmpty()) {
				this._children.first()._active = true;
			}
		}
		else {
	...
	ensureVisible() {
		...
		let found = this.uid !== 0 && (this._selected || this._children.filter(elem => elem._selected).count() > 0);
