...
class Controller extends Observer {
	...
	_show(sender, uri) {
		let pathSegments = URI.parse(uri).path.split('/');
	...
	cancel(sender, elementValue) {
		let router = this.repo.get(Router.name);
		if (this.view.isDirty === undefined) { // No dirty view found
	...
	children(sender, elementValue) {
		let router = this.repo.get(Router);
		router.goto(`list/${element.cls}?${element.property}=${element.uid}`);
	}
}
