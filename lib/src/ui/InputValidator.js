...
class InputValidator {
	...
	validate(element) {
		...
		try {
			...
			let format = element.dataformat;

			if (this._methods.has(type)) {
				let method = this._methods.get(type);
				input.value = method.call(this, input.value, format);
			}
	...
	required(input) {
		if (input.trim() === '') {
			throw new ValidationException("Skal udfyldes");
		}
		return input;
	}

	datetime(input, format) {
		let msg = "Dato og tid er ikke gyldig, da dato og klokkeslæt skal være adskilt af et mellemrum, fx 100717 1145";
	 ...
	 */
	time(input, format) {
		let msg = "Tidspunkt er ikke gyldigt, fx angives som 1045 eller 10:45.";
		let value = input.trim();
		if (value === '') {
			return input;
		}
	...
	date(input, format) {
		...
		if (isNaN(parseInt(value))) {
			throw new ValidationException("Dato er ikke gyldig. Brug formatet ddmmyy, DD-MM-YYYY o.l.");
		}
		switch(value.length) {
			case 1:
			case 2:
			...
			case 3:
			case 4:
				parsed.setMonth(parseInt(value.substring(2)) - 1, parseInt(value.substring(0, 2)));
				break;
			case 5:
			case 6:
				...
				parsed.setFullYear(yy,parseInt(value.substring(2, 4)) - 1, parseInt(value.substring(0, 2)));
				break;
			case 8:
				parsed.setFullYear(parseInt(value.substring(4)), parseInt(value.substring(2, 4)) - 1, parseInt(value.substring(0, 2)));
				break;
			default:
				throw new ValidationException("Dato kan ikke fortolkes. Brug formatet ddmmyy, DD-MM-YYYY o.l.");
		}
		let date = moment(parsed, format);
		return date.format(format);
	}
