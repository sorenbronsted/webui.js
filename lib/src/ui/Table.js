...
class Table extends ElementWrapper {
	constructor(view, table, factory) {
		super(view, table);

		this._factory = factory;
		this._none = 0;
		this._css = view.cssDelegate.table;
		this._headElements = collect([]);
		this._orderBy = null;
		...
		table.tHead.querySelectorAll('th[data-property]').forEach(element => {
			let elementWrapper = new Th(view, element);
			this._headElements.push(elementWrapper);
	...
	populate(sender, rows) {
		if (this._class !== sender || !rows || rows.objects == null || rows.objects.constructor.name !== 'Collection') {
			return;
		}
		// Shortcut rows because table is only interested in the collection of objects
		rows = rows.objects;
		...
		fragment.querySelectorAll('[data-property]').forEach(element => {
			this._rowElements.push(this._factory.make(this._view, element));
		});
	}
