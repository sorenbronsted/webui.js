...
class View extends Subject {
	...
	constructor(window, name, html, cssDelegate = new CssDelegate(), bindId ="content", formatter = new Formatter(),
		...
		this._fragment.querySelectorAll('[data-property], button[name], table[data-class], datalist[data-class], nav[data-class]').forEach(element => {
			if (element.tagName === 'TH') { //th has data-property attributes, but the table class handles this element
				return;
			}
			else if (element.tagName === 'A' && element.getAttribute('data-type') === 'menu') { // handled by nav class
				return;
			}
	...
	get isVisible() {
		return this._binding.childElementCount > 0 && this._binding.firstElementChild.dataset.view === this._viewName;
	}
	...
	show() {
		...
		this._binding.appendChild(this._fragment);
	...
	_createFragment(html) {
		...
		div.innerHTML = html;
		div.dataset.view = this._viewName;
		return div;
	}
}
exports.View = View;